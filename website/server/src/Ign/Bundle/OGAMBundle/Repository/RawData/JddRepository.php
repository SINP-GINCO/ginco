...
use Ign\Bundle\OGAMBundle\Entity\RawData\JddField;
...
class JddRepository extends EntityRepository  {
	...
	public function getActiveJdds(Provider $provider = null, User $user = null) {
		...
		$dql = "SELECT j FROM OGAMBundle:RawData\Jdd j WHERE j.status NOT IN ('deleted')";
	...
	public function getActiveJddsFields($jddId = null, User $user = null) {
		...
		$dql = "SELECT j FROM OGAMBundle:RawData\JddField j WHERE j.jdd = :jddId AND j.key NOT LIKE 'metadataCAId' ";
		$dql .= "ORDER BY j.jdd";
		$query = $this->getEntityManager()->createQuery($dql);
