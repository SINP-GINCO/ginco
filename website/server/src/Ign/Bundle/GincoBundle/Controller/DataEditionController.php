...
use Ign\Bundle\GincoBundle\Form\AjaxType;
...
class DataEditionController extends GincoController {
	...
	protected function getDataFromRequest($request) {
		...
		$data = $this->get('ginco.generic_service')->buildGenericTableFormat($schema, $format);
	...
	public function showEditDataAction(Request $request, $data = null, $message = '') {
		$genericModel = $this->get('ginco.manager.generic');
		$mode = 'EDIT';
		...
		$childrenTableLabels = $this->get('doctrine.orm.metadata_entity_manager')
			->getRepository('IgnGincoBundle:Metadata\TableTree')
			->getChildrenTableLabels($data->getTableFormat());
		
		return $this->render('IgnGincoBundle:DataEdition:edit_data.html.php', array(
			'dataId' => $data->getId(),
		...
		if ($childrenCount > 0) {
			...
			foreach ($data->all() as $field) {
				if ($field->getMetadata()
					...
					$uploadDir = $this->get('ginco.configuration_manager')->getConfig('image_upload_dir', '/var/www/html/upload/images');
			...
			if (!empty($ancestors)) {
				$parent = $ancestors[0];
				$redirectURL = $this->get('ginco.helper.editlink')->getEditLink($parent)['url'];
	 ...
	 * @todo make all fields within IgnGincoBundle:FormTypes\..
	 */
	...
	protected function getEditDataForm(GenericTableFormat $data, $mode) {
		...
		foreach ($data->getIdFields() as $tablefield) {
			...
			$formField = $this->get('ginco.generic_service')->getTableToFormMapping($tablefield);
			if (null !== $formField) {
	...
	public function ajaxGetEditFormAction(Request $request, $id = null) {
		...
		$requestId = $this->get('doctrine')
			->getRepository('Ign\Bundle\GincoBundle\Entity\Mapping\Request', 'mapping')
...
//		$ser->normalize($data); // FIXME : treewalker force loading proxy element ...
	...
	protected function getQueryService() {
		return $this->get('ginco.query_service');
	}
