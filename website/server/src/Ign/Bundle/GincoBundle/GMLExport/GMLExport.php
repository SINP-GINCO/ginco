...
use Ign\Bundle\GincoBundle\GMLExport\DEEModel;
...
use Ign\Bundle\OGAMBundle\Entity\Generic\GenericFieldMapping;
...
class GMLExport {
	 ...
	 */
	function getMetadataEntityManager() {
		return $this->get('doctrine.orm.metadata_entity_manager');
	}

	/**
	 * GMLExport constructor.
	 */
	public function __construct($logger, $locale, $configuration, $em, $emm, $genericService, $queryService) {
		...
		// Instantiate twig
		$loader = new \Twig_Loader_Filesystem(dirname(__FILE__) . '/../Resources/views/GMLExport');
	 ...
	 * @return boolean
	 */
	public function generateDeeGml($jddId, $fileName, $jobId = null, $userLogin) {
		...
		// Get the submission id (TODO : ids...)
		$em = $this->rawDataModel;
		$jddRepo = $em->getRepository('OGAMBundle:RawData\Jdd', 'rawdata');
		$jdd = $jddRepo->find($jddId);
		// $submissionId = $jdd->getActiveSubmissions()->first()->getId();
		$submissionId = '29';
		$modelId = $jdd->getModel()->getId();
		
		$this->logger->debug('$submissionId : ' . $submissionId . ' $modelId : ' . $modelId);
		
		// Get the dataset Id
		$submissionRepo = $em->getRepository('OGAMBundle:RawData\Submission', $schema);
		$submission = $submissionRepo->find($submissionId);
		$datasetId = $submission->getDataset()->getId();
		
		// -- Create a query object : the query must find all lines with given submission_id, TODO: submission_ids
		// And print a list of all fields in the model
		$queryForm = new QueryForm();
		$queryForm->setDatasetId($datasetId);
		
		$emm = $this->metadataModel;
		$tableFieldRepo = $emm->getRepository('OGAMBundle:Metadata\TableField', 'metadata');
		$tableFields = $tableFieldRepo->getTableFieldsForModel($modelId);
		
		$formFieldRepo = $emm->getRepository('OGAMBundle:Metadata\FormField', 'metadata');
		...
		foreach ($formFields as $formField) {
			...
			switch ($formField->getData()->getData()) {
				case 'SUBMISSION_ID':
					$queryForm->addCriterion($formField->getFormat()
						->getFormat(), 'SUBMISSION_ID', $submissionId);
					break;
			}
		...
		if ($total != 0) {
			...
			for ($i = 0; $i < $batches; $i ++) {
				...
				foreach ($results as $line) {
					$resultLine = array();
					// TODO : simplify ? On avait déjà les tableFields avant
