...
try {
	...
	$pdo->exec("UPDATE referentiels.liste_referentiels SET
			version = 'v12',
			updated_at = now()::date,
			url = 'https://inpn.mnhn.fr/telechargement/referentielEspece/taxref/12.0/menu'
	...
	$pdo->exec("
		CREATE OR REPLACE FUNCTION raw_data.sensitive_v12()
		RETURNS trigger AS
		\$BODY\$
	...
	foreach ($tableNames as $index => $value) {
		...
		$indexName = substr("idx_cdref_{$tableName}", 0, 64) ;
		...
		$pdo->exec("
			CREATE TRIGGER sensitive_v12$tableName 
			BEFORE UPDATE OF cdnom, cdref
			ON raw_data.$tableName 
			FOR EACH ROW EXECUTE PROCEDURE sensitive_v12()
		");
		...
		$casAbis = $pdo->exec("UPDATE raw_data.$tableName SET
				...
				taxoalerte = '1'
			FROM all_changes ac
			WHERE cdrefcalcule = ac.valeur_init
		...
		$casB1 = $pdo->exec("UPDATE raw_data.$tableName SET
				...
				taxoalerte = '1'
			FROM all_changes ac
			WHERE cdnomcalcule = ac.valeur_init
		...
		$casB0 = $pdo->exec("UPDATE raw_data.$tableName SET
			...
			WHERE cdnomcalcule = ac.valeur_init
			AND ac.type_change = 'RETRAIT'
			AND ac.cd_raison_suppression IS NULL
		");
		...
		$pdo->exec("DROP TRIGGER sensitive_v12$tableName ON raw_data.$tableName") ;
	...
	$pdo->exec("DROP FUNCTION raw_data.sensitive_v12()") ;
